...
async function loadAdminData() {
    ...
    // Load Appointments for Queue
    const apptRes = await fetch(`${API_URL}/appointments/all`, {
        headers: { 'Authorization': `Bearer ${token}` }
    });
    const appointments = await apptRes.json();
    const queueList = document.getElementById('adminQueueList');
    queueList.innerHTML = appointments.filter(a => a.status !== 'completed').map(a => `<li>${a.date} ${a.time} - ${a.patient.name} with ${a.doctor.name} (${a.status})</li>`).join('');
