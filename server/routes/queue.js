...
router.get('/status', async (req, res) => {
    try {
        ...
        res.json({
            current: current ? { queueNumber: current.queueNumber, patientName: current.patient.name } : null,
            next: next ? { queueNumber: next.queueNumber, patientName: next.patient.name } : null,
            waiting: waiting.map(a => ({ queueNumber: a.queueNumber, patientName: a.patient.name }))
        });
    } catch (err) {
